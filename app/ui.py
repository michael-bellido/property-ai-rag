"""
Presentation layer for Property AI: bilingual UI text, CSS, and every
render_*() function. Deliberately has no dependency on config.py, rag.py,
prompts.py, or llm.py — this module only ever renders what it's handed
(strings, dicts, lists), so it can be reasoned about (and tested) without
touching retrieval, prompting, or the LLM client at all.
"""
import html
...
# st.chat_input — see app.main()), and the ONLY customization left is
...
def render_sidebar():
    with st.sidebar:
        ...
        # independent of this toggle (see prompts.SYSTEM_PROMPT's
        # "Language requirements"). "Property AI" itself never translates.
        lang = current_lang()
