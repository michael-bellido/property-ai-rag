...
unbounded number of questions. See _has_reached_session_limit() and its
use in handle_question() in app.py. Pure integer comparison, no LLM call,
...
def test_zero_questions_asked_has_not_reached_the_limit():
    assert app._has_reached_session_limit(0, limit=20) is False


def test_below_the_limit_has_not_reached_it():
    assert app._has_reached_session_limit(19, limit=20) is False
...
def test_at_the_limit_has_reached_it():
    ...
    assert app._has_reached_session_limit(20, limit=20) is True


def test_past_the_limit_has_reached_it():
    assert app._has_reached_session_limit(25, limit=20) is True


def test_uses_the_apps_default_limit_when_not_overridden():
    just_under = app.QUESTION_LIMIT_PER_SESSION - 1
    assert app._has_reached_session_limit(just_under) is False
    assert app._has_reached_session_limit(app.QUESTION_LIMIT_PER_SESSION) is True
