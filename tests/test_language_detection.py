...
English: instead of trusting the model's own language detection,
prompts.py detects the question's language in Python and injects an
explicit, per-question directive naming that language. These tests run
instantly — no LLM call, no network, no API key required.
"""
import prompts


def test_detects_clearly_english_question():
    assert prompts.guess_question_language("Can I buy property as a non-resident?") == "English"


def test_detects_clearly_spanish_question():
    assert prompts.guess_question_language("¿Puedo comprar una propiedad siendo no residente?") == "Spanish"
...
def test_detects_spanish_from_accented_characters_alone():
    ...
    assert prompts.guess_question_language("¿Está disponible mañana?") == "Spanish"
...
def test_ambiguous_short_input_returns_none():
    ...
    assert prompts.guess_question_language("ok") is None


def test_empty_or_blank_input_returns_none():
    assert prompts.guess_question_language("") is None
    assert prompts.guess_question_language("   ") is None


def test_build_language_directive_names_the_detected_language():
    directive = prompts.build_language_directive("What villas do you have with a pool?")
    assert "English" in directive


def test_build_language_directive_falls_back_when_undetectable():
    directive = prompts.build_language_directive("ok")
